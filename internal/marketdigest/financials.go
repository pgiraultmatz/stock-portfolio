...
func financialOptions(options RankingOptions) RankingOptions {
	switch options.Limit {
	case 3, 5, 10, 20, 30, 50:
	default:
		options.Limit = 50
	}
...
// GenerateFinancialReport has no technical-alert input and performs no I/O.
func GenerateFinancialReport(options RankingOptions) string {
	options = financialOptions(options)
	var sb strings.Builder
	writeHeader(&sb)
	fmt.Fprintf(&sb, "<h2>Revue financière - %s</h2>\n", options.Now.Format("02/01/2006"))
	writeFinancialSection(&sb, options)
	sb.WriteString(`<p class="footer">Source : Yahoo Finance. Generated by stock-checker.</p></body></html>`)
	return sb.String()
}
